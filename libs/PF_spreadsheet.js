...
( function( jexcel, mw ) {
	...
	jexcel.prototype.getEditorForAutocompletion = function( inputType, x, y, autocompletedatatype, autocompletesettings, cell, type ) {
		...
		const editor = pfSpreadsheetAutocomplete ? widget.$element[0] : document.createElement(type);
...
( function ( $, mw, pf ) {
	...
	$( '.pfSpreadsheet' ).each( function() {
		...
		let spreadsheetID = $(this).attr( 'id' );
		const columns = [];
			...
			function populateSpreadsheet() {
				...
				function rowAdded2( $instance, spreadsheetId ) {
					...
					for ( columnNum = 0; columnNum < defaultColumnParams.length; columnNum++ ) {
						...
						const $curCell = $newRow.children('td').eq( columnNum + 1 );
 ...
 return false;
				...
				if ( editMultiplePages !== undefined ) {
					const numberOfColumns = $(table).find('thead tr:first-child td').not('.jexcel_selectall').length;
					let fieldNum = 0;
