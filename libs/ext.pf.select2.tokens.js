	...
	tokens_proto.apply = function( element ) {
		...
		$(inputData.$container[0]).on("keyup",(e) => {
			...
			if( e.keyCode === 9 ){
				...
				if( valHighlighted !== undefined ){
					...
					const highlightedData = $( valHighlighted ).data( 'data' );
				...
				const hasOption = $input.find( 'option' ).filter( function() {
					return this.value === optionValue;
				} ).length > 0;
				if ( !hasOption ) {
					const newOption = new Option( optionLabel, optionValue, false, false );
		...
		if ( element.attr( "existingvaluesonly" ) !== "true" ) {
			element.parent().on( "dblclick", "li.select2-selection__choice", ( event ) => {
				...
				const clickedData = $target.data( 'data' );
				const clickedValue = clickedData && clickedData.id !== undefined ? clickedData.id : $target[0].title;
				const clickedLabel = clickedData && clickedData.text !== undefined ? clickedData.text : $target[0].title;
	...
	tokens_proto.setOptions = function() {
		...
		if( !this.existingValuesOnly ){
			...
			opts.insertTag = function( data, tag ) {
				...
				const normalizedTagId = normalizeValue( tag.id );
				const normalizedTagText = normalizeValue( tag.text );
				let duplicateTag = false;
				data.forEach( ( item ) => {
					const normalizedItemId = normalizeValue( item.id );
					const normalizedItemText = normalizeValue( item.text );
			...
			if ( autocompletesettings === 'external data' ) {
				...
				if ( data !== undefined && data !== null ) {
					for (const key in data) {
						const optionId = Array.isArray( data ) ? data[key] : key;
						const localOptionText = data[key];
						values.push({
			...
			$.ajax({
				...
				success: function(value) {
					...
					value.pfautocomplete.forEach( (item) => {
						const dependentOptionText = item.displaytitle !== undefined ? item.displaytitle : item.title;
						values.push({
							id: item.title,
