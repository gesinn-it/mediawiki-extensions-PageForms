...
FormField (field configuration)
        │
        ▼
FormPrinter (input resolution and rendering)
        │
        ▼
HTML widget output
        │
...
All input types are registered with `FormPrinter`.
...
The value `combobox` is stored in the `FormField` instance and later
used by `FormPrinter` to determine the input class.
...
`FormField::additionalHTMLForInput()`.
...
The system uses two registration mechanisms, defined in `PF_showOnSelect.js`:
