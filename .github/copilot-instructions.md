...
The following features will be removed in a future version. The same
rules apply to all of them:

- Do **not** write new tests for these code paths.

- Do **not** document this behaviour in any new or updated
  documentation.

- When coverage gaps in existing code are caused solely by these code
  paths, treat those gaps as irrelevant — do not create tests to fill
  them.

- Annotate code blocks with a PHPUnit coverage ignore comment so that
  they are excluded from coverage metrics:
...
Support for the Cargo and PageSchemas extensions will be removed.
Cargo-specific code blocks are scattered through `includes/` and
