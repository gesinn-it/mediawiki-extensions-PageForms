...
On MW 1.43 + SMW 6.0.1, `LinksUpdate` (which writes SMW property
annotations) is a `TransactionRoundAwareUpdate`. It only executes after
`LBFactory::commitPrimaryChanges()` is called to close a database
transaction round. In a fresh PHPUnit process, this commit never happens
automatically for the very first test case, so property annotations are
not written to the SMW store before the test assertions run.

**Effect**: The first property-dependent JSONScript test file run in
isolation (e.g. via `--filter`) on a fresh MW 1.43 container returns
empty API results. Subsequent test files in the same PHPUnit process
pass because the transaction round was committed by the MW test
