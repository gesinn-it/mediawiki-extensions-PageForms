...
- `PFValuesUtils::resolveDisplayTitle()`: strip HTML tags from parsed display titles before returning them; MediaWiki stores `displaytitle` page props as rendered HTML (e.g. `<i>MyPage</i>` for `''MyPage''`), so the tokens autocomplete dropdown was showing raw HTML markup instead of the plain text label (#34)
